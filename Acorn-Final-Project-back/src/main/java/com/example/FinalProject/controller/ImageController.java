...
public class ImageController {
	 ...
	 *	"/upload/yyy.png"
	 *	"/upload/zzz.gif"
	 *	패턴의 요청이 오면 실제 해당 이미지를 읽어서 실제 이미지 데이터를 응답하는 컨츠롤러 메소드 만들기
	 *	경로변수 = 동적인 경로를 읽는다 {경로변수}
	 */
	...
	public ResponseEntity<InputStreamResource> image(@PathVariable("imageName") String name) throws IOException{
		...
		//이미지의 이름을 이용해서 응답할 이미지가 어디에 있는지 전체 겨로를 구성한다.
		...
		//이미지 데이터를 응답하는 ResponseEntity 객체를 구성해서 리턴해준다
